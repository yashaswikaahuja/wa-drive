async function fillFormFieldsSequential(mapping, filledBySource, portalAdapters, allFields) {
  ...
  // T5 — reliable live debug: queue + long-lived port to SW.
  // sendMessage alone drops field.* under fill load; only start/end used to arrive.
  const _fillRunId = 'fill:' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  let _debugPort = null;
  let _debugQueue = [];
  let _debugFlushTimer = null;
  function _ensureDebugPort() {
    if (_debugPort) return _debugPort;
    try {
      if (typeof chrome === 'undefined' || !chrome.runtime || !chrome.runtime.connect) return null;
      _debugPort = chrome.runtime.connect({ name: 'cc_fill_debug' });
      _debugPort.onDisconnect.addListener(function () {
        _debugPort = null;
      });
    } catch (e) {
      _debugPort = null;
  ...
  function _flushDebugQueue() {
    if (!_debugQueue.length) return;
    const batch = _debugQueue.splice(0, 40);
    try {
      const port = _ensureDebugPort();
      if (port) {
        port.postMessage({ type: 'FILL_DEBUG_BATCH', events: batch });
        if (_debugQueue.length) _scheduleDebugFlush();
        return;
      }
    } catch (e) {
      _debugPort = null;
  ...
  function _scheduleDebugFlush() {
    if (_debugFlushTimer) return;
    _debugFlushTimer = setTimeout(function () {
  ...
  function _emitFillDebug(event, payload) {
    ...
    _debugQueue.push(evt);
    // Start/end and large batches flush immediately; field.* coalesce ~40ms
    if (event === 'fill.start' || event === 'fill.end' || _debugQueue.length >= 6) {
      if (_debugFlushTimer) {
        clearTimeout(_debugFlushTimer);
        _debugFlushTimer = null;
      }
      _flushDebugQueue();
    } else {
  ...
  // ── Select / cascade helpers (AJAX vs static + honest fail codes) ─────────
  function _isPlaceholderOption(o) {
    if (!o) return true;
  ...
  function _realOptions(el) {
    if (!el || !el.options) return [];
    return Array.from(el.options).filter((o) => !_isPlaceholderOption(o));
  }
  function _sampleOptions(el, n) {
    n = n || 8;
    return _realOptions(el).slice(0, n).map((o) => ({
      value: String(o.value || '').slice(0, 40),
      text: String(o.text || '').trim().slice(0, 60),
    }));
  }
  function _readSelectActual(el) {
    if (!el || el.tagName !== 'SELECT') return { actualValue: null, actualOptionValue: null };
    const opt = el.options && el.options[el.selectedIndex];
    if (!opt || _isPlaceholderOption(opt)) {
  ...
  function _selectLoadMode(el) {
    if (!el || el.tagName !== 'SELECT') return 'unknown';
    return _realOptions(el).length > 0 ? 'static' : 'ajax';
  }
  function _cascadeSemanticKey(label, profileKey, selector) {
  ...
  function _pushSelectRecord(base) {
  ...
  /**
   * SEQUENTIAL SETTLE — the real AJAX contract.
   * Dependencies are not only select→select; radio can unlock selects and vice-versa.
   * After each act, wait until network is quiet (or budget) so the *next* field
   * sees the post-AJAX DOM. We do not need to name "which pair is ajax".
   *
   * kind:
   *   text   — short local settle only
   *   choice — radio/checkbox/toggle (may trigger show/hide + AJAX)
   *   select — dropdown (may load options for later fields)
   *   button — navigation / section reveal
   */
  async function settleAfterAct(kind, opts) {
    opts = opts || {};
    const budget = typeof opts.budgetMs === 'number' ? opts.budgetMs : _ajaxWaitBudgetMs;
    if (kind === 'text') {
    ...
    _ajaxWaitBudgetMs = Math.max(0, _ajaxWaitBudgetMs - used);
  ...
  async function waitForSelectOptionsSequential(selector, maxMs) {
    maxMs = Math.min(maxMs || 6000, Math.max(400, _ajaxWaitBudgetMs || 400));
    const t0 = Date.now();
    ...
    _ajaxWaitBudgetMs = Math.max(0, _ajaxWaitBudgetMs - (Date.now() - t0));
    return el;
  }
  ...
  async function fillSequential() {
    for (const [selector, fieldData] of entries) {
      ...
      const _t0 = Date.now();
      // Label is semantic authority; selector is only the act target
      const _fieldCtx = {
        type,
      ...
      if (fieldData.type === 'button') {
        ...
        // Sequential: next field may depend on this click (section reveal / AJAX)
        ...
        // Previous act (radio/select) may still be loading options into this control
        ...
        // ── SEQUENTIAL native <select> ──────────────────────────────────────
        // Do not special-case "cascade graph" as the only AJAX path.
        // Any previous field (radio, select, checkbox) may have unlocked options.
        // Contract: act only when strategy can run; then settle before next field.
        ...
        // Empty options → wait for sequential settle (prior radio/select may load them)
        if (_realOptions(liveEl).length === 0) {
          const exhausted = _ajaxWaitBudgetMs <= 0 || _ajaxNotLoadedCount >= 5;
          const cap = exhausted ? 500 : Math.min(6000, Math.max(500, _ajaxWaitBudgetMs));
          console.log('[CC] sequential-wait-options:', selector, 'budget=', _ajaxWaitBudgetMs, 'cap=', cap);
        ...
        // One sequential retry if strategy failed but we still have budget (late AJAX append)
        if (!applyRes.ok && _ajaxWaitBudgetMs > 2000 && _ajaxNotLoadedCount < 3) {
          attempt = 2;
        ...
        // Strategy worked? verified DOM is the source of truth
        if (matchOk) {
          filled += 1;
          if (semKey) {
        ...
        // ── File input — never open OS file chooser (Chrome user-activation rule) ──
        if (value && (value.startsWith('http://') || value.startsWith('https://'))) {
          ...
          // No URL / only filename — operator must pick file manually
          _ccRecords.push({
            selector,
            value: value || null,
            type: 'file',
        ...
        // Standard field (text / radio-click / checkbox / etc.) — sequential
        // Radio/checkbox may unlock later selects via AJAX; always settle after choice.
        const isChoice =
          type === 'radio-click' ||
          type === 'radio' ||
          type === 'radio-group' ||
        ...
        try {
          const _r = fillOne(selector, value, type) || 0;
          // Settle before verify so frameworks finish applying the act
          ...
          // Strategy truth: act returned ok AND (for text) value landed; for radio, checked state
          let _trulyFilled = false;
          if (isChoice && _r === 1) {
            // Radio/checkbox: strategy worked if element is checked (or group has selection)
            if (_el2 && (_el2.type === 'radio' || _el2.type === 'checkbox')) {
              _trulyFilled = !!_el2.checked;
            } else {
              _trulyFilled = _ver.ok || _r === 1;
            }
            // If verify said value-rejected but checked is true, trust DOM checked
