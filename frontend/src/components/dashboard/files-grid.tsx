...
export function FilesGrid({ files, newIds, viewMode, selectedIds, onToggleSelect, onPreview, onDownload, onPrint, onDelete }: Props) {
  if (files.length === 0) {
    return (
  ...
  if (viewMode === 'list') {
    return (
      <div className="flex flex-col gap-2">
        {files.map(f => (
          <div key={f.id}
            className={`flex items-center gap-4 p-3 bg-card border rounded-lg hover:border-accent/50 cursor-pointer transition-colors ${selectedIds?.has(f.id) ? 'border-accent' : 'border-border'}`}>
            {onToggleSelect && (
              <input type="checkbox" checked={selectedIds?.has(f.id) ?? false}
                onChange={() => onToggleSelect(f.id)}
                onClick={e => e.stopPropagation()}
                className="w-4 h-4 accent-accent shrink-0" />
            )}
            <div className="flex-1 flex items-center gap-4 min-w-0" onClick={() => onPreview(f)}>
