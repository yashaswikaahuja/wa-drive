      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        ...
        {!selectionMode && (
          <div className="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition">
            ...
            <button onClick={(e) => { e.stopPropagation(); const driveId = msg.fileUrl?.match(/[?&]id=([a-zA-Z0-9_-]+)/)?.[1]; if (!driveId) return; window.open(`${API_URL}/drive/download/${driveId}?print=1`, '_blank'); }} className="text-[10px] px-2 py-0.5 rounded-md bg-green-600/20 text-green-400 hover:bg-green-600/30">Print</button>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="h-[calc(100vh-48px)] flex">
      ...
      {viewerFile && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={handleCloseViewer}>
          ...
          <button onClick={() => { const driveId = viewerFile.fileUrl?.match(/[?&]id=([a-zA-Z0-9_-]+)/)?.[1]; if (!driveId) return; window.open(`${API_URL}/drive/download/${driveId}?print=1`, '_blank'); }} className="absolute top-4 right-28 px-3 py-1.5 rounded-lg bg-green-600/80 text-white text-xs">🖨 Print</button>
