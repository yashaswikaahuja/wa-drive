      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        ...
        {!selectionMode && (
          <div className="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition">
            ...
            <button onClick={(e) => { e.stopPropagation(); const driveId = msg.fileUrl?.match(/[?&]id=([a-zA-Z0-9_-]+)/)?.[1]; if (!driveId) return; const ext2 = msg.fileName?.split('.').pop()?.toLowerCase()||''; const isImg = ['jpg','jpeg','png','gif','webp','bmp'].includes(ext2); if (isImg) { const w = window.open('','_blank'); if(w){w.document.write(`<html><body style="margin:0;display:flex;justify-content:center;align-items:center;min-height:100vh"><img src="https://drive.google.com/thumbnail?id=${driveId}&sz=w1200" style="max-width:100%;max-height:100vh" onload="window.print()"/></body></html>`);w.document.close();} } else { const w = window.open('','_blank'); if(w){w.document.write(`<html><body style="margin:0;height:100vh"><embed src="${API_URL.replace('/api','')}/api/drive/download/${driveId}" type="application/pdf" width="100%" height="100%" onload="setTimeout(()=>window.print(),500)"/></body></html>`);w.document.close();} } }} className="text-[10px] px-2 py-0.5 rounded-md bg-green-600/20 text-green-400 hover:bg-green-600/30">Print</button>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="h-[calc(100vh-48px)] flex">
      ...
      {viewerFile && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={handleCloseViewer}>
          ...
          <button onClick={() => { const driveId = viewerFile.fileUrl?.match(/[?&]id=([a-zA-Z0-9_-]+)/)?.[1]; if (!driveId) return; const ext2 = viewerFile.fileName?.split('.').pop()?.toLowerCase()||''; const isImg = ['jpg','jpeg','png','gif','webp','bmp'].includes(ext2); if (isImg) { const w = window.open('','_blank'); if(w){w.document.write(`<html><body style="margin:0;display:flex;justify-content:center;align-items:center;min-height:100vh"><img src="https://drive.google.com/thumbnail?id=${driveId}&sz=w1200" style="max-width:100%;max-height:100vh" onload="window.print()"/></body></html>`);w.document.close();} } else { const w = window.open('','_blank'); if(w){w.document.write(`<html><body style="margin:0;height:100vh"><embed src="${API_URL.replace('/api','')}/api/drive/download/${driveId}" type="application/pdf" width="100%" height="100%" onload="setTimeout(()=>window.print(),500)"/></body></html>`);w.document.close();} } }} className="absolute top-4 right-28 px-3 py-1.5 rounded-lg bg-green-600/80 text-white text-xs">🖨 Print</button>
