      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        ...
        {!selectionMode && (
          <div className="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition">
            ...
            <button onClick={async (e) => { e.stopPropagation(); const driveId = msg.fileUrl?.match(/[?&]id=([a-zA-Z0-9_-]+)/)?.[1]; if (!driveId) return; const res = await api.get(`/drive/download/${driveId}`, {responseType:'blob'}); const blob = new Blob([res.data], {type: res.headers['content-type']||'application/pdf'}); const url = URL.createObjectURL(blob); const iframe = document.createElement('iframe'); iframe.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:99999;border:none;background:white'; iframe.src = url; document.body.appendChild(iframe); iframe.onload = () => { setTimeout(()=>{ iframe.contentWindow?.focus(); iframe.contentWindow?.print(); },500); iframe.onclick = () => { URL.revokeObjectURL(url); iframe.remove(); }; }; const close = document.createElement('button'); close.textContent = '✕ Close'; close.style.cssText = 'position:fixed;top:10px;right:10px;z-index:100000;padding:8px 16px;background:#333;color:white;border:none;border-radius:8px;cursor:pointer;font-size:14px'; close.onclick = () => { URL.revokeObjectURL(url); iframe.remove(); close.remove(); }; document.body.appendChild(close); }} className="text-[10px] px-2 py-0.5 rounded-md bg-green-600/20 text-green-400 hover:bg-green-600/30">Print</button>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="h-[calc(100vh-48px)] flex">
      ...
      {viewerFile && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={handleCloseViewer}>
          ...
          <button onClick={async () => { const driveId = viewerFile.fileUrl?.match(/[?&]id=([a-zA-Z0-9_-]+)/)?.[1]; if (!driveId) return; const res = await api.get(`/drive/download/${driveId}`, {responseType:'blob'}); const blob = new Blob([res.data], {type: res.headers['content-type']||'application/pdf'}); const url = URL.createObjectURL(blob); const iframe = document.createElement('iframe'); iframe.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:99999;border:none;background:white'; iframe.src = url; document.body.appendChild(iframe); iframe.onload = () => { setTimeout(()=>{ iframe.contentWindow?.focus(); iframe.contentWindow?.print(); },500); }; const close = document.createElement('button'); close.textContent = '✕ Close'; close.style.cssText = 'position:fixed;top:10px;right:10px;z-index:100000;padding:8px 16px;background:#333;color:white;border:none;border-radius:8px;cursor:pointer;font-size:14px'; close.onclick = () => { URL.revokeObjectURL(url); iframe.remove(); close.remove(); }; document.body.appendChild(close); }} className="absolute top-4 right-28 px-3 py-1.5 rounded-lg bg-green-600/80 text-white text-xs">🖨 Print</button>
