...
export function setLastImage(buf) { lastImageBuffer = buf; }
...
export async function detectFace(imageBuffer) {
    ...
    if (!key || !secret) {
        console.warn('[FacePP] API keys not set — skipping');
        return null;
    }
    ...
    const result = await new Promise((resolve) => {
        ...
        const req = https.request({ hostname: FACEPP_HOST, path: FACEPP_PATH, method: 'POST', headers: form.getHeaders() }, (res) => {
            let data = '';
            ...
            res.on('end', () => {
                try {
                    ...
                    const face = json.faces.reduce((best, f) => f.face_rectangle.width * f.face_rectangle.height > best.width * best.height
                        ? f.face_rectangle : best, json.faces[0].face_rectangle);
 ...
 */
export async function cropAndAlignFace(input, targetW, targetH, options = {}) {
...
//# sourceMappingURL=faceDetect.js.map