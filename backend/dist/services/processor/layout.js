...
const MM = (mm) => Math.round(mm * 300 / 25.4);
...
async function preparePhoto(input, pw, ph) {
    return cropAndAlignFace(input, pw, ph, { pad: 0.9 });
}
/** Build a white text strip to place BELOW the photo — photo is never modified */
async function buildTextStrip(pw, text, font = 'bold') {
    const lines = [text.name, text.date].filter(Boolean);
    if (!lines.length && !text.signature)
        return null;
...
export async function generatePassportSheet(photoBuffer, preset = '4x6-8', spec = 'standard', text, font = 'bold') {
...
export async function generateSingleSheet(photoBuffer, spec = 'standard') {
...
export async function generateAadhaarLayout(buffers) {
    if (buffers.length !== 2)
