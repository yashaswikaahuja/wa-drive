...
export class WhatsAppService {
    constructor() {
        ...
        this.customerNames = new Map(); // phone → name
    }
    setSocketIO(io) { this.io = io; }
    setDriveToken(token) { this.driveAccessToken = token; }
    getDriveToken() { return this.driveAccessToken; }
    getStatus() { return this.isConnected; }
    getQrCode() { return this.lastQrCode; }
    getCustomerName(phone) { return this.customerNames.get(phone) ?? `Guest ${phone.slice(-4)}`; }
    async init() {
    ...
    async handleMedia(message) {
        ...
        if (!phone || phone.length < 4) {
            const rawFrom = message._data?.from ?? message.from ?? '';
        ...
        if (!media) {
            ...
            if (!retried) {
                console.warn('[WhatsApp] downloadMedia() returned null after retry');
                return;
            }
    ...
    async processMedia(message, media, phone, customerName, contact) {
        ...
        const mimeType2Label = { 'image/': 'photo', 'video/': 'video', 'audio/': 'audio' };
        const typeLabel = Object.entries(mimeType2Label).find(([k]) => mimetype.startsWith(k))?.[1] ?? 'file';
        const rawName = media.filename ?? message._data?.filename ?? '';
        ...
        try {
            ...
            try {
                ...
                if (profilePicUrl)
                    console.log(`[WhatsApp] Profile pic via @c.us for ${phone}`);
                else
