...
router.get('/households', authMiddleware, async (req, res) => {
    try {
...
router.patch('/persons/:id', authMiddleware, async (req, res) => {
    const { fields, displayLabel, relationship } = req.body;
    try {
        const { rows } = await pool.query("SELECT data FROM profiles WHERE id = $1 AND workspace_id = $2 AND deleted_at IS NULL", [req.params.id, req.user.workspaceId]);
        if (!rows.length)
