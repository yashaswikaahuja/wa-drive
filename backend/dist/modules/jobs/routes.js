...
router.post('/', authMiddleware, async (req, res) => {
    ...
    if (!profileId || !serviceType)
        return res.status(400).json({ error: 'profileId and serviceType required' });
    try {
        const { rows } = await pool.query("INSERT INTO jobs (workspace_id, user_id, profile_id, service_type, metadata, notes) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, status, created_at", [req.user.workspaceId, req.user.userId, profileId, serviceType, metadata ? JSON.stringify(metadata) : null, notes || null]);
...
router.post('/:id/dispatch', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (sessionId && (totalFilled !== undefined || records)) {
            ...
            if (totalFilled !== undefined) {
                updates.push(`total_filled = $${pi}`);
                params.push(totalFilled);
                pi++;
            }
            if (totalFailed !== undefined) {
                updates.push(`total_failed = $${pi}`);
                params.push(totalFailed);
                pi++;
            }
            if (records) {
                updates.push(`records = $${pi}::jsonb`);
