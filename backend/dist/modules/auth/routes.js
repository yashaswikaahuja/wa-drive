...
router.post('/google', authLimiter, async (req, res) => {
    ...
    try {
        ...
        let userRow = (await pool.query('SELECT id, workspace_id, name, role, status FROM users WHERE email = $1 AND deleted_at IS NULL', [email])).rows[0];
        if (!userRow) {
            ...
            try {
                ...
                const u = await client.query("INSERT INTO users (workspace_id, email, name, role, status, password_hash) VALUES ($1,$2,$3,'admin','active','') RETURNING id, workspace_id, name, role", [workspaceId, email, name || email]);
...
router.post('/register', authLimiter, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const userResult = await client.query("INSERT INTO users (workspace_id, email, phone, password_hash, name, role) VALUES ($1,$2,$3,$4,$5,'admin') RETURNING id", [workspaceId, email || null, phone || null, hash, name || null]);
