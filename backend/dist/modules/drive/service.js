...
oauth2Client.on('tokens', async (tokens) => {
    try {
        ...
        console.warn('[Drive] Token persist failed:', e.message);
    }
});
export async function loadDriveTokenFromDB() {
    try {
        ...
        console.warn('[Drive] Token load failed:', e.message);
    }
}
...
export async function getDriveForWorkspace(wsId) {
    const tokRow = await pool.query("SELECT key, value FROM workspace_secrets WHERE workspace_id = $1 AND key IN ('drive_access_token','drive_refresh_token')", [wsId]);
    const wsTokens = {};
    tokRow.rows.forEach((r) => (wsTokens[r.key] = r.value));
...
export async function uploadFileToDrive(drive, buffer, fileName, mimetype, phone, senderName) {
