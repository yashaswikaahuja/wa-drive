...
import { oauth2Client, getDrive } from './service.js';
const router = Router();
...
router.get('/files/ws', authMiddleware, async (req, res) => {
    try {
        const r = await pool.query('SELECT id, file_name as "fileName", customer_id as "customerId", customer_name as "customerName", file_url as "fileUrl", uploaded_at as "timestamp", profile_pic_url as "dpUrl" FROM drive_files WHERE workspace_id = $1 ORDER BY uploaded_at DESC', [req.user.workspaceId]);
        if (r.rows.length > 0)
        ...
        console.error('[drive/files/ws]', e.message);
        res.json([]);
    }
});
// Download proxy
router.get('/download/:fileId', (req, res, next) => {
    ...
    try {
        ...
        res.status(e.code || 500).json({ error: 'Download failed: ' + e.message });
    }
});
