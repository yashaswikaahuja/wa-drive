...
router.post('/upload', upload.single('file'), async (req, res) => {
    ...
    if (uploadWsId) {
        try {
            ...
            if (drive)
                console.log('[Hub] Using workspace Drive tokens for', uploadWsId.slice(0, 8));
        }
        catch (e) {
            console.warn('[Hub] WS token load error:', e.message);
        }
    }
    if (!drive) {
    ...
    try {
        const dedupCheck = await pool.query("SELECT id FROM drive_files WHERE workspace_id=$1 AND customer_id=$2 AND file_name=$3 AND uploaded_at > now() - interval '60 seconds'", [uploadWsId, phone, fileName]);
    ...
    try {
        ...
        // DB insert
        try {
            await pool.query('INSERT INTO drive_files(id,workspace_id,file_name,customer_id,customer_name,file_url,uploaded_at,profile_pic_url) VALUES($1,$2,$3,$4,$5,$6,now(),$7) ON CONFLICT(id) DO NOTHING', [fileId, uploadWsId, fileName, phone, senderName, `https://drive.google.com/thumbnail?id=${fileId}&sz=w200`, profilePicUrl || null]);
            if (profilePicUrl)
                await pool.query('UPDATE drive_files SET profile_pic_url = $1 WHERE workspace_id = $2 AND customer_id = $3', [profilePicUrl, uploadWsId, phone]);
            if (senderName)
                await pool.query('UPDATE drive_files SET customer_name = $1 WHERE workspace_id = $2 AND customer_id = $3 AND customer_name != $1', [senderName, uploadWsId, phone]);
        }
        catch (e) {
