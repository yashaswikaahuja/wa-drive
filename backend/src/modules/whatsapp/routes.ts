...
router.post('/event', (req, res) => {
  ...
  const io = getIO();
  if (event === 'qr') io.emit('qr', { qr, workspaceId });
  else if (event === 'connected') io.emit('connection:status', { connected: true, phone, workspaceId });
  else if (event === 'disconnected') io.emit('connection:status', { connected: false, workspaceId });
  res.json({ ok: true });
});
