 ...
 * Table-based, inline-styled, web-safe fonts → renders consistently across Gmail/Outlook/Apple Mail.
 * Each builder returns { subject, html, text } (text = plaintext fallback for deliverability/a11y).
 */
...
// Outer shell: cream backdrop + white rounded card with the CyberControl logo lockup + footer.
...
<body style="margin:0;padding:0;background:#F6F2E9;-webkit-text-size-adjust:100%;">
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">CyberControl</div>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" style="width:480px;max-width:100%;background:#ffffff;border:1px solid #E7E0D2;border-radius:16px;overflow:hidden;">
        <tr><td style="padding:28px 32px 4px 32px;">
          <table role="presentation" cellpadding="0" cellspacing="0"><tr>
            ...
            <td style="padding-left:11px;font-size:19px;font-weight:700;color:#131A2B;letter-spacing:-0.3px;">Cyber<span style="color:#E97612;">Control</span></td>
          </tr></table>
        </td></tr>
        ...
        <tr><td style="padding:20px 32px 28px 32px;border-top:1px solid #F0EADD;">
          <p style="margin:0;font-size:11px;color:#9A9384;line-height:1.6;">You received this because this address was used on CyberControl. If it wasn't you, you can safely ignore this email.</p>
          <p style="margin:8px 0 0 0;font-size:11px;color:#B8B1A2;">&copy; CyberControl &middot; cybercontrol.fun</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export function otpEmail(code: string): { subject: string; html: string; text: string } {
  const inner = `
    ...
    <p style="margin:22px 0 0 0;font-size:12.5px;color:#8A8474;line-height:1.6;">For your security, never share this code. CyberControl staff will never ask you for it.</p>`;
  return {
    subject: `${code} is your CyberControl verification code`,
    html: shell(inner),
    text: `Your CyberControl verification code is ${code}. It expires in 10 minutes. Never share this code. If you didn't request it, ignore this email.`,
  };
}
...
export function welcomeEmail(name?: string | null): { subject: string; html: string; text: string } {
  ...
  const inner = `
    ...
    <p style="margin:0 0 18px 0;font-size:14px;color:#5B5648;line-height:1.6;">Your CyberControl workspace is ready. Here's how to get started:</p>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" style="margin:24px 0 4px 0;"><tr>
      <td style="background:#F98A1E;border-radius:10px;">
        <a href="${APP_URL}" style="display:inline-block;padding:12px 26px;font-size:14px;font-weight:700;color:#ffffff;text-decoration:none;">Open CyberControl &rarr;</a>
      </td>
    </tr></table>`;
  return {
    subject: 'Welcome to CyberControl 🎉',
    html: shell(inner),
    text: `Welcome${name ? ' ' + name.split(' ')[0] : ''}! Your CyberControl workspace is ready.\n\n` +
