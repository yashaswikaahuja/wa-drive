...
type AdaptersStore = Record<string, Record<string, Adapter>>;

function load(): AdaptersStore {
  if (!existsSync(ADAPTERS_PATH)) return {};
  try { return JSON.parse(readFileSync(ADAPTERS_PATH, 'utf8')); } catch { return {}; }
}
function save(data: AdaptersStore) {
...
router.post('/:hostname', (req: Request, res: Response) => {
  const { componentClass, triggerSelector, optionsContainer, optionSelector, verifySelector } = req.body as Partial<Adapter>;
...
router.patch('/:hostname/:componentClass', (req: Request, res: Response) => {
  const { success, stale } = req.body as { success?: boolean; stale?: boolean };
  const store = load();
