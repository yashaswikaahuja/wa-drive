...
async function getFormColumns(pool: any): Promise<Set<string>> {
...
async function safeSelectList(pool: any): Promise<string> {
  const cols = await getFormColumns(pool);
...
router.get('/', async (req: any, res) => {
  try {
    ...
    const cols = await getFormColumns(req.pool);
    const selectList = await safeSelectList(req.pool);
...
router.get('/:id', async (req: any, res) => {
  try {
    const { rows } = await req.pool.query('SELECT * FROM forms WHERE id = $1', [req.params.id]);
...
router.patch('/:id', async (req: any, res) => {
  ...
  try {
    const { rowCount, rows } = await req.pool.query(
...
router.post('/', async (req: any, res) => {
  ...
  const values: any[] = [name, short_name, portal, url, lifecycleToStatus(lifecycle)];
