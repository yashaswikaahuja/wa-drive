 ...
 */
import type { AliasMap, ServerFieldMapping } from './types.ts';
...
export function getFieldAliases(serverMappings?: ServerFieldMapping[] | null): AliasMap {
  const merged: AliasMap = Object.assign({}, FIELD_ALIASES);
  const server =
    serverMappings ||
    (typeof window !== 'undefined' && window._ccServerFieldMappings) ||
    null;
