...
function getDisplayText(root) {
  ...
  const el = root.querySelector('.select-type') || root.querySelector('.value-area') ||
    ...
    root.querySelector('[class*="mat-select-value"] span') || root.querySelector('.mat-select-value-text,.mat-mdc-select-value-text') ||
    root.querySelector('button > span:first-child') || root.querySelector('button span');
  if (el) return el.textContent.trim();
  const clone = root.cloneNode(true);
  clone.querySelectorAll('ul,ol,[class*="options"],[class*="dropdown-list"],[class*="drop-list"],[class*="menu"],[class*="items"]').forEach(e => e.remove()); clone.querySelectorAll('[class*="placeholder"]:not([class*="value"])').forEach(e => e.remove());
...
function findRoot(doc, field) {
  ...
  if (typeof field.domIndex === 'number') {
    ...
    if (!root) {
      ...
      )).filter(el => el.tagName !== 'SELECT' && el.tagName !== 'INPUT' && !el.querySelector('select') && (el.querySelector('button > span') || el.querySelector('li,[class*=option]') || el.getAttribute('role') === 'combobox' || el.className.includes('dropdown') || el.className.includes('select') || el.className.includes('picker') || el.className.includes('chosen') || el.tagName === 'MAT-SELECT' || el.tagName === 'NG-SELECT'));
  ...
  const cands = Array.from(doc.querySelectorAll(
    '[class*="dropdown"],[class*="select"],[class*="picker"],[class*="chosen"],[class*="react-select"],[class*="v-select"],[class*="p-dropdown"],[class*="ui-selectmenu"],[class*="ng-select"],mat-select,ng-select,div.relative'
  )).filter(el => el.tagName !== 'SELECT' && el.tagName !== 'INPUT' && !el.querySelector('select') && (el.querySelector('button > span') || el.querySelector('li,[class*=option]') || el.getAttribute('role') === 'combobox' || el.className.includes('dropdown') || el.className.includes('select') || el.className.includes('picker') || el.className.includes('chosen') || el.tagName === 'MAT-SELECT' || el.tagName === 'NG-SELECT'));
...
function testDobSplit(label, id, name, dob) {
  ...
  if (ident.includes('day') && (ident.includes('birth')||ident.includes('dob')||new Set(ident.split(/[_\s]+/).filter(Boolean)).size === 1)) return { val: parseInt(dobDay).toString(), type: 'day' };
  if (ident.includes('month') && (ident.includes('birth')||ident.includes('dob')||new Set(ident.split(/[_\s]+/).filter(Boolean)).size === 1)) return { val: monthNames[parseInt(dobMonth)], type: 'month' };
  if (ident.includes('year') && (ident.includes('birth')||ident.includes('dob')||new Set(ident.split(/[_\s]+/).filter(Boolean)).size === 1)) return { val: dobYear, type: 'year' };
  return null;
}
...
for (const site of SITES) {
  ...
  let updated = false;
  // For mat-select, simulate Angular adding mat-select-value-text span
  const matPlaceholder = root.querySelector('.mat-select-placeholder');
  if (matPlaceholder) { const sp = doc.createElement('span'); sp.className='mat-select-value-text'; sp.textContent=site.selectValue; matPlaceholder.parentNode.replaceChild(sp, matPlaceholder); updated = true; }
  // For Vue button-based: update button > span
  if (!updated) { const btnSpan = root.querySelector('button > span:first-child') || root.querySelector('button span'); if (btnSpan) { btnSpan.textContent = site.selectValue; updated = true; } }
  if (!updated) { for (const s of DISPLAY_SELS) { const el = root.querySelector(s); if (el) { el.textContent = site.selectValue; updated = true; break; } } }
