  ...
  try {
    ...
    const result = await chrome.scripting.executeScript({
      ...
      func: async (profile, profileId, backendUrl, accessToken, groqKey, llmBaseUrl, llmModel) => {
        ...
        try {
          ...
          if (saved && typeof saved === 'object') {
            ...
            for (const f of formFields) {
              ...
              // Saved mappings are already keyed by normalized LABEL (sk = norm(f.label))
              if (act.kind === 'value') {
                mapping[f.selector] = {
                  value: act.value,
                  type: f.type,
                  matchBy: 'label',
                  profileKey: s.profileKey || null,
                  label: f.label,
                };
                setFbs();
                ...
                if (grp === 'radio' && Array.isArray(f.optionSelectors)) {
                  ...
                  if (optSel) {
                    ...
                    fbs[optSel] = { label: f.label, profileKey: s.profileKey, source: 'mapping', matchBy: 'label' };
                    handled.add(f.selector);
                  }
                } else {
                  mapping[f.selector] = {
                    value: act.option,
        ...
        // Fuzzy fill remaining — LABEL-PRIMARY (mapper matches label → profile key;
        // selector is only the fill target, never the semantic authority).
        ...
        if (unmapped.length) {
          ...
          for (const [s, v] of Object.entries(fz)) {
            ...
            fbs[s] = {
              label: (v && v.label) || (ff && ff.label) || s,
              profileKey: (v && v.profileKey) || null,
              source: 'fuzzy',
              matchBy: (v && v.matchBy) || 'label',
            };
          }
          console.log(
            '[CC] label-primary fuzzy mapped',
            Object.keys(fz).length,
            'fields; matchBy=label count=',
            Object.values(fz).filter((x) => x && x.matchBy === 'label').length
          );
        }
        ...
        // Optional AI mapping — soft timeout (never reject / never console.warn → Chrome Errors)
        // Exclude radio/checkbox — handled by rule engine / sequential fill.
        ...
        if (unmappedAI.length > 0 && groqKey) {
          try {
            const aiPromise = aiMatch(unmappedAI, profile, groqKey, llmBaseUrl, llmModel).catch(() => ({}));
            const softTimeout = new Promise((resolve) => setTimeout(() => resolve({ __timeout: true }), 8000));
            const aiMapping = await Promise.race([aiPromise, softTimeout]);
            if (aiMapping && !aiMapping.__timeout) {
              for (const [sel, val] of Object.entries(aiMapping)) {
                if (!mapping[sel]) {
                  mapping[sel] = val;
                  fbs[sel] = { label: (val && val.label) || 'ai', source: 'ai', matchBy: 'ai' };
                }
              }
            } else {
              console.debug('[CC] aiMatch soft-timeout — continuing without AI map');
            }
          } catch (e) {
            console.debug('[CC] aiMatch skipped:', (e && e.message) || e);
          }
        }

        // ── Option validation: demote only when STATIC options are known and mismatch.
        // AJAX cascades often have empty / placeholder options at extract time —
        // do NOT demote those (that was mis-labeled as "mapping missing" later).
        const isPlaceholderOpt = (o) => {
          const t = String(o || '').toLowerCase().trim();
          return !t || t === '--' || t.includes('select') || t.includes('choose') || t.includes('loading');
        };
        for (const f of formFields) {
          ...
          const realOpts = (f.options || []).filter((o) => !isPlaceholderOpt(o));
          if (realOpts.length > 0) {
            // Static list available: validate
            const matched = realOpts.some(o => {
              const on = o.toLowerCase().trim();
              return on === val || on.includes(val) || val.includes(on);
            });
            if (!matched) {
              console.log('[CC] demoted unmatched STATIC dropdown:', f.label, '→', mapping[f.selector].value);
              delete mapping[f.selector]; delete fbs[f.selector];
            }
          } else if ((f.type === 'ng-dropdown' || f.type === 'mat-select') && src !== 'mapping' && src !== 'manual') {
            // Custom widget with no options yet: only trust saved mappings
            console.log('[CC] demoted unverified ng-dropdown:', f.label, '→', mapping[f.selector].value, '(source:', src, ')');
            delete mapping[f.selector]; delete fbs[f.selector];
          }
          // else: native select with empty options → AJAX child; keep mapping for executor wait
        }
        ...
        if (stillBlank.length > 0 && groqKey && typeof ccAiResolveValues === 'function') {
          try {
            ...
            // Soft timeout: resolve {} so fill continues — never reject (no Chrome Errors spam)
            const rPromise = ccAiResolveValues(pending, profile, groqKey, llmBaseUrl, llmModel).catch(() => ({}));
            const rTimeout = new Promise((resolve) => setTimeout(() => resolve({ __timeout: true }), 8000));
            const resolved = await Promise.race([rPromise, rTimeout]);
            if (resolved && !resolved.__timeout) {
              let n = 0;
              for (const [sel, info] of Object.entries(resolved)) {
                if (mapping[sel] || !info || info.value == null) continue;
                ...
                if (grp === 'radio' && Array.isArray(f.optionSelectors)) {
                  ...
                  if (oi >= 0 && f.optionSelectors[oi]) {
                    mapping[f.optionSelectors[oi]] = { value: info.value, type: 'radio-click', matchBy: 'ai', label: f.label };
                    fbs[f.optionSelectors[oi]] = { label: f.label, source: 'ai-resolve', matchBy: 'ai' };
                    handled.add(f.selector); n++;
                  }
                } else {
                  mapping[sel] = { value: info.value, type: f.type, matchBy: 'ai', label: f.label };
                  fbs[sel] = { label: f.label, source: 'ai-resolve', matchBy: 'ai' };
                  n++;
                }
              }
              if (n) console.log('[CC] ai-resolve filled', n, 'residual field(s)');
            } else {
              console.debug('[CC] ai-resolve soft-timeout — fill continues with mapped fields only');
            }
          } catch (e) {
            console.debug('[CC] ai-resolve skipped:', (e && e.message) || e);
          }
        }
        ...
        // Append honest "unmapped" diagnostics — NOT always "no-mapping".
        // Profile may have district; another selector may already own that cascade key.
        const filledSelectors = new Set(records.map(r => r.selector));
        const semKeyOf = (label, pk) => {
          const s = ((pk || '') + ' ' + (label || '')).toLowerCase();
          if (/state|राज्/.test(s) && !/sub/.test(s)) return 'state';
          if (/sub[_\s-]*div|अनुमंडल/.test(s)) return 'sub_division';
          if (/district|जिला|jila/.test(s)) return 'district';
          if (/block|प्रखंड|tehsil/.test(s)) return 'block';
          if (/panchayat|पंचायत/.test(s)) return 'panchayat';
          if (/village|gram|ग्राम/.test(s)) return 'village';
          if (/police|thana|थाना/.test(s)) return 'police_station';
          if (/husband|पति/.test(s)) return 'husband';
          if (/father|पिता/.test(s)) return 'father';
          if (/mother|माता/.test(s)) return 'mother';
          if (/email|ईमेल/.test(s)) return 'email';
          if (/mobile|phone|मोबाइल/.test(s)) return 'mobile';
          return '';
        };
        const attemptedBySem = {};
        for (const r of records) {
          const sk = semKeyOf(r.label, r.profileKey);
          if (!sk) continue;
          if (!attemptedBySem[sk]) attemptedBySem[sk] = [];
          attemptedBySem[sk].push(r);
        }
        // Profile keys that have non-empty values (mapping available in product sense)
        const profileKeysPresent = new Set(
          Object.keys(profile || {}).filter((k) => {
            const v = profile[k];
            if (v == null || v === '') return false;
            if (typeof v === 'object' && v && 'value' in v) return v.value != null && v.value !== '';
            return true;
          })
        );
        ...
        for (const f of formFields) {
          ...
          if (mapping[f.selector]) {
            // Mapped but executor never reported — surface as gap
            records.push({
              selector: f.selector,
              label: f.label,
              type: f.type,
              value: mapping[f.selector].value,
              result: 'skipped',
              failReason: 'executor_no_record',
              strategy: 'planner',
              source: fbs[f.selector]?.source || 'mapping',
              profileKey: fbs[f.selector]?.profileKey || null,
              ts: Date.now(),
            });
            continue;
          }
          const sem = semKeyOf(f.label, fbs[f.selector]?.profileKey);
          const twins = sem ? attemptedBySem[sem] || [] : [];
          let reason = 'selector_not_bound';
          let note = null;
          if (twins.length) {
            reason = 'duplicate_hierarchy';
            note = 'same semantic key attempted on ' + twins.length + ' other control(s)';
          } else if (
            (f.type === 'ng-dropdown' || f.type === 'mat-select' || f.type === 'select' || f.type === 'dropdown') &&
            (profileKeysPresent.has('district') || profileKeysPresent.has('state') || profileKeysPresent.has('block'))
          ) {
            // Cascade-ish field with profile geo data but this selector unbound
            reason = 'selector_not_bound';
            note = 'profile may have value; this selector never entered mapping{}';
          } else if (f.type === 'radio' || f.type === 'radio-group' || f.type === 'checkbox' || f.type === 'mat-radio') {
            reason = 'selector_not_bound_for_' + f.type;
          } else {
            reason = 'no_profile_value_for_selector';
          }
          // Keep legacy token only as secondary for old UIs
          records.push({
            selector: f.selector,
            label: f.label,
            type: f.type,
            ...
            result: 'unmapped',
            failReason: reason,
            failReasonLegacy: 'no-mapping',
            note,
            strategy: 'planner',
            source: 'none',
            semanticKey: sem || null,
            ts: Date.now(),
          });
        }
        ...
        // Honest fail count: skips + unmapped that were not pure duplicates
        const totalFailed = records.filter(r =>
          r.result === 'skipped' ||
          r.result === 'error' ||
          r.result === 'failed' ||
          (r.result === 'unmapped' && r.failReason !== 'duplicate_hierarchy')
        ).length;
